...
func Test_LoadGPGPrivateKey(t *testing.T) {
	...
	t.Run("fail", func(t *testing.T) {
		...
		assert.Error(t, err)
		_, err = cp.LoadGPGPrivateKey(now, []byte("pkcs11:manufacturer=test;id=123;type=private;serial=123"))
		assert.Error(t, err)
...
func Test_LoadSigner(t *testing.T) {
	...
	t.Run("pkcs11URI", func(t *testing.T) {
		...
		signer, err := cp.LoadSigner([]byte(uri))
		require.NoError(t, err)
	...
	t.Run("fail", func(t *testing.T) {
		_, err = cp.LoadSigner([]byte(""))
		assert.Error(t, err)
		_, err = cp.LoadSigner([]byte("pkcs11"))
		assert.Error(t, err)
		_, err = cp.LoadSigner([]byte("pkcs11:manufacturer=test"))
		assert.Error(t, err)
		_, err = cp.LoadSigner([]byte("pkcs11:manufacturer=test;id=123;type=private;serial=123"))
		assert.Error(t, err)
		_, err = cp.LoadSigner([]byte("pkcs11:manufacturer=SoftHSM;id=123;type=private;serial=123"))
		assert.Error(t, err)
	})
}
