...
func TestHttp_RequestLogger(t *testing.T) {
	...
	exp := []string{"xhttp", " src=ServeHTTP, " + testLogPrefix, "", "GET", "/foo", testRemoteIP, testRemotePort, strconv.Itoa(http.StatusBadRequest), "1.1", strconv.Itoa(len(testResponseBody)), something, "\"no-agent\"\n"}
